...
const CERTIFICATE_DEFAULT = { _id: '', userId: '', courseId: '', status: 'In Progress' }

const MyCertifications: React.FC = () => {
  const [certificates, setCertificates] = useState<Certificate[]>([]);
  const [certificate, setCertificate] = useState<Certificate>(CERTIFICATE_DEFAULT);
  ...
  const fetchCertificates = async () => {
    setIsLoading(true);
    try {
      let data = await getReq('/api/certificates/user') as any[];
...
const fetchCompletedCourses = async () => {
  try {
    const data = await getReq('/api/courses/completed');
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (courseId) {
      setCertificate({ ...certificate, courseId })
  ...
  const submitCertificate = async (item: Certificate) => {
    ...
    try {
      if (item._id)
        await putReq(`/api/certificates/${item._id}`, item)
      else
  ...
  const downloadCertificate = async (item: Certificate) => {
    ...
    try {
      ...
      console.log(data)
      downloadBase64Image(data.certificate, 'certicate.jpg');
    } catch (err) {
    ...
    return (
      <React.Fragment>
        {item.status != 'Approved' ?
          <PrimeButton icon="pi pi-pencil" style={{ borderRadius: '50px' }} outlined className="mr-2" onClick={() => editCertificate(item)} />
          : <PrimeButton icon="pi pi-download" style={{ borderRadius: '50px' }} outlined severity="success" className="mr-2" onClick={() => downloadCertificate(item)} />
        }
        <PrimeButton icon="pi pi-trash" style={{ borderRadius: '50px' }} outlined severity="danger" onClick={() => confirmDeleteCertificate(item)} />
      </React.Fragment>
    );
  }
  ...
  const deleteCertificateDialogFooter = (
    <React.Fragment>
      <PrimeButton label="No" icon="pi pi-times" outlined className='mr-3' onClick={hideDeleteCertificateDialog} />
      <PrimeButton label="Yes" icon="pi pi-check" severity="danger" onClick={deleteCertificate} />
    </React.Fragment>
  );
  ...
  return (
    <DashboardComponent>
      ...
      <LoaderBoundary isLoading={isLoading}>
        {showCertificate ? <>
          <CertificateComponent certificate={certificate} submitCallback={submitCertificate} cancelCallback={cancelCertificate} />
        </> : <>
  ...
  >
    Request Certificate
      ...
      <Dialog visible={deleteCertificateDialog} style={{ width: '32rem' }}
        breakpoints={{ '960px': '75vw', '641px': '90vw' }} header="Confirm"
        modal footer={deleteCertificateDialogFooter} onHide={hideDeleteCertificateDialog}>
